...
@testset "Simple Economy Tests" begin
    ...
    @testset "Conservation Laws" begin
        ...
        @test conservation.loan_balance
        # Note: bank_balance is false as we need to adjust initial balances
    end
    ...
    @testset "Patterns" begin
        ...
        @test verify_pattern(wage_pattern)
        @test verify_pattern(loan_pattern)
        @test verify_pattern(trade_pattern)
    end
    ...
    @testset "Colimits" begin
        ...
        colimit_obj, colimit_morphisms = calculate_colimit(wage_pattern)
        @test verify_colimit(wage_pattern, colimit_obj, colimit_morphisms)
        @test startswith(colimit_obj, "colim_")
        @test length(colimit_morphisms) == 2
    end

    @testset "Account Balances" begin
        # Test initial balances
        @test company_bank.balance == 1000.0
        @test company_loan.balance == -500.0
        @test household_bank.balance == 500.0
        @test bank_company_bank.balance == -1000.0
    end

    @testset "Transaction Values" begin
        # Test transaction amounts
        @test wage_payment.value == 100.0
        @test loan_payment.value == 50.0
        @test good_purchase.value == 80.0
    end
end